...
class LyricsService {
  ...
  static Future<LyricsResult?> _getLyricsFromKuGou({
    ...
    try {
      ...
      final searchUri = Uri.parse(_kuGouSearchUrl).replace(queryParameters: {
        'ver': '1',
        'man': 'yes',
        'client': 'mobi',
      ...
      final searchResponse = await http.get(searchUri).timeout(const Duration(seconds: 10));
      ...
      // Download the lyrics
      final downloadUri = Uri.parse(_kuGouDownloadUrl).replace(queryParameters: {
        'ver': '1',
        'client': 'pc',
        'id': id,
        'accesskey': accessKey,
      ...
      final downloadResponse = await http.get(downloadUri).timeout(const Duration(seconds: 10));
  ...
  static Future<List<LyricsResult>> searchLyrics({
    ...
    try {
      ...
      final uri = Uri.parse('$_lrcLibBaseUrl/search').replace(queryParameters: queryParams);
...
class LyricsResult {
  ...
  factory LyricsResult.fromJson(Map<String, dynamic> json, {String providerName = 'LrcLib'}) {
    return LyricsResult(
      id: json['id'] ?? 0,
...
class LyricLine {
  ...
  List<LyricWord> parseWords(Duration? nextLineTimestamp) {
    ...
    final lineEndTime = nextLineTimestamp ?? (timestamp + const Duration(seconds: 5));
